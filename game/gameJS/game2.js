...
function game2() {
    ...
    if (gameStart == false){
        ...
        Matter.Composite.translate(engine.world, createVector((window.innerWidth - disWidth) / -2, (window.innerHeight - disHeight) / -2), false);
            ...
            if (subGame == 0) {
                ...
                gameObj.push(Bodies.rectangle(disWidth * 0.6 + disWidth * 0.2 * random(), disHeight / 10 * 4.5, disWidth / 10 * 1.2, disHeight / 10 * 0.15, {isStatic: true, inertia: Infinity, angle: 0}));
            } else if (subGame == 1) {
                engine.gravity.y = disHeight / 1150 * 1.5;
                character.jumpForce = 0.95 * character.size;
                gameObj.push(Bodies.rectangle(disWidth * 0.15 + disWidth * 0.7 * random(), disHeight / 10 * 7.8, disWidth / 10 * 2, disHeight / 10 * 0.15, {isStatic: true, inertia: Infinity, angle: 0}));
                gameObj.push(Bodies.rectangle(disWidth * 0.15 + disWidth * 0.7 * random(), disHeight / 10 * 6.15, disWidth / 10 * 2, disHeight / 10 * 0.15, {isStatic: true, inertia: Infinity, angle: 0}));
                gameObj.push(Bodies.rectangle(disWidth * 0.15 + disWidth * 0.7 * random(), disHeight / 10 * 4.5, disWidth / 10 * 2, disHeight / 10 * 0.15, {isStatic: true, inertia: Infinity, angle: 0}));
            }
            ...
            World.add(engine.world, [gameObj[3]]);
    ...
    for (var i = 0; i < gameObj.length; i++) {
        ...
        Matter.Composite.translate(engine.world, createVector((window.innerWidth - disWidth) / -2, (window.innerHeight - disHeight) / -2), false);
            if (timeDelta < timeDelta_update && Math.round((timeDelta_update % 4.5) * 10) / 10 == (i * 1.5 + 0.1)) {
                if (i == 0 || gameObjExist[3] == 1) {
                    gameObjExist[3] = 1;
                    if (gameObjExist[i] == 1) {
                        ...
                        if (subGame == 1) {
                            Body.setPosition(gameObj[i], createVector(disWidth * 0.15 + disWidth * 0.7 * random(), disHeight / 10 * (7.8 - 1.65 * i)));
                        }
                        World.add(engine.world, [gameObj[i]]);
                        gameObjExist[i] = 1;
                    }
                }
            }
    ...
    push();
        ...
        for (var i = 0; i < gameObj.length - 1; i++) {
            if (gameObjExist[i] == 1) drawVertices(gameObj[i].vertices);
        }
    pop();
